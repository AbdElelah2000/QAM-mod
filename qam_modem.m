...
N = 4096; %FTT samples
sampling_rate = 100.0e3;% rate in Hz
tstep = 1/sampling_rate;%time steps
tmax = N*tstep/2;

tmin = -tmax;%max and min of time
tt = tmin:tstep:tmax-tstep;
fmax = sampling_rate/2; 
fmin = -fmax;%max and min of frequency
fstep = (fmax-fmin)/N;
...
%First message signal in Time domain
figure(1)
Hp1 = plot(tt,mt1);
set(Hp1,'LineWidth',2)
Ha = gca;
...
%Second message signal in Time domain
figure(2)
Hp1 = plot(tt,mt2);
set(Hp1,'LineWidth',2)
Ha = gca;
...
%Spectrum of the QAM signal in frequency domain
figure(6)
